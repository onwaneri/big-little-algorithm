...
const EMPTY: BannedPair = { freshman: "", sophomore: "" };

export default function BannedPairsTable({ banned, freshNames, sophNames, onChange }: Props) {
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm text-gray-500">
          Specify freshman-sophomore pairs that should NOT be matched together.
        </p>
        <button
          onClick={addRow}
      ...
      {banned.length === 0 ? (
        <p className="text-sm text-center text-gray-400 italic py-4 border border-dashed border-gray-200 rounded-lg">
          No banned pairs — all freshman-sophomore matches are allowed.
        </p>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-red-200 shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-red-50 text-red-800 uppercase text-xs">
              <tr>
                <th className="px-3 py-2 text-left">Freshman</th>
                <th className="px-3 py-2 text-left">Sophomore</th>
              ...
              {banned.map((ban, idx) => (
                <tr key={idx} className="hover:bg-red-50">
                  ...
                  <td className="px-3 py-1.5">
                    <select
                      value={ban.freshman}
                      onChange={(e) => update(idx, { freshman: e.target.value })}
                  ...
                  <td className="px-3 py-1.5">
                    <select
                      value={ban.sophomore}
                      onChange={(e) => update(idx, { sophomore: e.target.value })}
                      className="w-full border border-gray-300 rounded px-1 py-1 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-red-400"
                    >
                      <option value="">— select —</option>
                      {sophNames
                        .filter((n) => n !== ban.freshman)
                        .map((n) => (
                          <option key={n} value={n}>
                            {n}
                          </option>
